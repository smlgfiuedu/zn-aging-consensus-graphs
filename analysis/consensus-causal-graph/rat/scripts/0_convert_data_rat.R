...
for(g in 1:ncol(organism.csv)){
  if(g==(ncol(organism.csv)-1)){
    
    organism.csv[,g] <- factor(organism.csv[,g], levels = c("Adequate", "Deficient"))
    
  }else if(g==ncol(organism.csv)){
    
    organism.csv[,g] <- factor(organism.csv[,g], levels = c("Baseline", "CR"))
